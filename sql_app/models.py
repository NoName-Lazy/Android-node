...
class Star(Base):
    ...
    uuid = Column(String, ForeignKey("users.uuid"))  # 外键，关联 users.uuid
    item_id = Column(Integer, ForeignKey("items.id"))  # 外键，关联 items.id
    item_title = Column(String)  # 文章标题
    star_time = Column(DateTime)  # 点赞时间